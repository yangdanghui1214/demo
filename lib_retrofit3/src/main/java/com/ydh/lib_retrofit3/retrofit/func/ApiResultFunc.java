...
package com.ydh.lib_retrofit3.retrofit.func;
...
import com.ydh.lib_retrofit3.util.Utils;
...
public class ApiResultFunc<T> implements Function<ResponseBody, BasicResponse<T>> {
    protected Type type;
    protected Gson gson;
    ...
    @Override
    public BasicResponse<T> apply(@NonNull ResponseBody responseBody) throws Exception {
        ...
        if (type instanceof ParameterizedType) {//自定义ApiResult
            ...
            try {
                ...
                if (clazz.equals(String.class)) {
                    //apiResult.setData((T) json);
                    //apiResult.setCode(0);
                    final BasicResponse result = parseApiResult(json, apiResult);
                    if (result != null) {
                        apiResult = result;
                        apiResult.setData((T) json);
                    } else {
