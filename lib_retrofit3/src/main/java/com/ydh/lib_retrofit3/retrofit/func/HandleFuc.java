 ...
 * @author 13001
 */
public class HandleFuc<T> implements Function<BasicResponse<T>, T> {
    @Override
    public T apply(@NonNull BasicResponse<T> tApiResult) {
        if (isOk(tApiResult)) {
     ...
     * @return 是否为正常返回
     */
    private boolean isOk(BasicResponse<T> tApiResult) {
        if (tApiResult == null) {
            return false;
        }
        return tApiResult.getCode() == 0;
    }
}
